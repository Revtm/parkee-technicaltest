...
public class PaymentController {
    ...
    @Autowired
    public PaymentController(PaymentService paymentService, ControllerUtils controllerUtils, CheckOutConverter checkOutConverter) {
    ...
    public ResponseEntity<PaymentResponseDto> submitTicketPayment(@RequestBody PaymentRequestDto request){
        try {
            ...
            PaymentResponseDto response = checkOutConverter.checkOutToCheckOutResponseDto(payment);

            HttpStatus httpStatus = controllerUtils.mappingHttpStatus(payment.getProcessStatus());
            return new ResponseEntity<>(response, httpStatus);
        }catch (Exception e){
            log.error("Error", e);
            PaymentResponseDto response = checkOutConverter.requestToFailedCheckOutResponseDto(request);
